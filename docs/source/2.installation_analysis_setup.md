...
2. Create and activate a screen-specific conda environment (~10 min)
   ...
   # install the mozzarellm cluster-annotation extra (optional, for 13.run_mozzarellm.sh)
...
On our local machine, this takes about 15 minutes.
   ...
   # run brieflow on test analysis data (add --zarr for OME-Zarr output)
   bash run_brieflow.sh
   cd ../../..
   pytest   # from inside the brieflow directory
   ```

To explore the test results in the [visualizer](https://brieflow.readthedocs.io/en/latest/4.visualization.html), run from `brieflow/tests/small_test_analysis` (after a `--zarr` run, use `BRIEFLOW_OUTPUT_PATH=brieflow_output_zarr/` and `CONFIG_PATH="config/config.yml config/config_zarr.yml"`):
...
`tests/small_test_analysis/screen.yaml` describes the test dataset and is a filled-in example of the `analysis/screen.yaml` your screen repository starts with.
