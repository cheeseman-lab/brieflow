...
Brieflow supports both TIFF and Zarr formats. To enable Zarr support, add the following to your `config.yml`:

```yaml
preprocess:
  output_formats: "zarr"  # or ["tiff", "zarr"] for both
  downstream_input_format: "zarr"  # optional, defaults to "tiff" if available
```

**Zarr Benefits:**
- Faster I/O for large datasets
- Cloud-native format for distributed computing
- OME-Zarr outputs for visualization in Napari
- Efficient multiscale pyramids for interactive viewing

See the [Config Glossary](config_glossary.md#zarr-support) for detailed Zarr configuration options.
