...
`source` picks the functional annotation the evidence bundles carry: `affinage` (the default, MozzareLLM's benchmark-selected source), `uniprot`, or `both`. No source backfills another, so a gene one source has nothing for reaches the model as a visible gap; stable accessions always come from UniProt.
