...
### Optional OME-Zarr exports

Preprocess can emit NGFF (`*.ome.zarr`) outputs alongside the usual TIFFs. Add an `ome_zarr` block under `preprocess` in your analysis config when you want those stores:

```yaml
preprocess:
  # existing preprocess keys …
  ome_zarr:
    enabled: true
    output_root: brieflow_output/preprocess/ome_zarr
    chunk_xy: 1024            # spatial chunk size (defaults to 1024 when omitted)
    scale_factors: [2, 4]
    version: "0.5"
    validate: true
    compression:              # optional; omit to store uncompressed chunks
      id: blosc
      cname: zstd
      clevel: 3
      shuffle: 2
...
If you leave `compression` out, the writer stores raw chunks (still 1024×1024). Any dict you supply is threaded directly into the OME-Zarr writer so you can choose other lossless Blosc codecs or levels. With `ome_zarr.enabled: true`, you can run `snakemake --until all_preprocess_ome_zarr` (or include that target in your normal preprocess run) to build the `.ome.zarr` plate stores.
