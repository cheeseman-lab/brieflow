...
**Configure**: add a `mozzarellm` section to `config.yml` and write one screen-context JSON per row of its combo table (`12.analyze.ipynb` writes them with `screen_context_for_combo`).
...
**Run**:
- **Local**: `sh 13.run_mozzarellm.sh` (remove `-n` for actual run)
