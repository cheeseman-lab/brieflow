...
Options `flow.sh` does not recognise are passed to `streamlit run`, for example `bash flow.sh viz --server.port 8502` to use another port.
...
| Pipeline Statistics | The `<screen>_stats.txt` report `12_analyze.py` writes to `brieflow_output/` |
...
Per-module counts and summaries from the `*_stats.txt` report in `brieflow_output/`, shown only when such a file is present.
`12_analyze.py` writes it as `<screen>_stats.txt`, where `<screen>` is `screen_title` from `screen.yaml` made file-safe (`screen_stats.txt` without one), and prints the same statistics in the notebook.
