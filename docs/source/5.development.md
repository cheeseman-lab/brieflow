...
The integration test runs the `Test Installation` code from [Installation & Analysis Setup](2.installation_analysis_setup.md) (step 3) in a GitHub action.

**Running tests locally before creating a PR:**

```bash
cd brieflow/tests/small_test_analysis/
bash run_brieflow.sh          # TIFF output
bash run_brieflow.sh --zarr   # OME-Zarr output
cd ../..
pytest                        # unit tests in brieflow/tests/
```

Run both output modes when a change touches file paths or image I/O.
...
These docs live in `docs/source/` as Markdown ([MyST](https://myst-parser.readthedocs.io)) and build with Sphinx and the [Furo](https://pradyunsg.me/furo/) theme.
A change that adds or removes a notebook parameter, a `flow.sh` option or a module should update the pages that describe it, including the [Glossary](glossary.md) and [Notebook Parameters](notebook_parameters.md).
Build locally and fix any warnings before opening a PR:
