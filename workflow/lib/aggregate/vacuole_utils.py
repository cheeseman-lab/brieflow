...
def aggregate_vacuole_data(
    ...
    if agg_strategy not in valid_strategies:
        raise ValueError(f"Unknown strategy: {agg_strategy}. Must be one of {valid_strategies}")
...
def _validate_merge_keys(df: pd.DataFrame, required_keys: List[str], df_name: str) -> None:
    """
...
def _aggregate_single(
    ...
    vacuole_cols = [col for col in vacuoles_df.columns if col not in vacuoles_merge_keys]
    ...
    if vacuole_cols:
        nan_df = pd.DataFrame(
            np.nan, 
            index=multi_vacuole_cells.index, 
            columns=vacuole_cols
        )
...
def _aggregate_all(
    ...
    vacuoles_numbered['vacuole_num'] = vacuoles_numbered.groupby(cells_merge_keys).cumcount() + 1
    ...
    for vac_num in range(1, max_vacuoles_global + 1):
        ...
        current_vacuoles = vacuoles_numbered[vacuoles_numbered['vacuole_num'] == vac_num].copy()
...
def _aggregate_average(
    ...
    numeric_cols = vacuoles_for_merge[vacuole_cols].select_dtypes(include=[np.number]).columns.tolist()
    ...
    if agg_dict:  # Only aggregate if there are columns to aggregate
        vacuoles_averaged = vacuoles_for_merge.groupby(cells_merge_keys).agg(agg_dict).reset_index()
        
        # Merge with cells data
        result_df = cells_df.merge(
            vacuoles_averaged,
            on=cells_merge_keys,
            how='left'
        )
