...
from lib.merge.hash import (
    get_vectors, nine_edge_hash, get_vc, nearest_neighbors, 
    gb_apply_parallel, extract_rotation
)
from lib.merge.merge import build_linear_model


def extract_nuclei_positions_from_stitched(stitched_image: np.ndarray, 
                                          cell_info_df: pd.DataFrame,
                                          metadata_df: pd.DataFrame,
                                          well: str, 
                                          tile_shifts: Dict[str, List[int]]) -> pd.DataFrame:
    """
...
def hash_stitched_well_locations(well_positions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(well_positions_df) < 4:
        print(f"Warning: Only {len(well_positions_df)} cells found, need at least 4 for triangulation")
        return pd.DataFrame()
...
def merge_stitched_wells(phenotype_positions: pd.DataFrame,
                        sbs_positions: pd.DataFrame, 
                        alignment: pd.Series,
...
def well_merge_pipeline(phenotype_info: pd.DataFrame,
                       ...
                       distance_threshold: float = 2.0) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    print(f"Found {len(phenotype_positions)} phenotype cells, {len(sbs_positions)} SBS cells")
    ...
    alignment_df = well_level_alignment(
        phenotype_hash, sbs_hash, phenotype_positions, sbs_positions,
        det_range, score_threshold
    )
    ...
    if (alignment['determinant'] < det_range[0] or 
        alignment['determinant'] > det_range[1] or
        alignment['score'] < score_threshold):
