...
def merge_stitched_cells(
    ...
    print(
        f"Using transformation: rotation det={np.linalg.det(rotation):.3f}, translation={translation}"
    )
    ...
    print(
        f"  Transformed phenotype: i=[{transformed_coords[:, 0].min():.0f}, {transformed_coords[:, 0].max():.0f}], j=[{transformed_coords[:, 1].min():.0f}, {transformed_coords[:, 1].max():.0f}]"
    )
    print(
        f"  SBS: i=[{sbs_coords[:, 0].min():.0f}, {sbs_coords[:, 0].max():.0f}], j=[{sbs_coords[:, 1].min():.0f}, {sbs_coords[:, 1].max():.0f}]"
    )
    ...
    print(
        f"Duplicate phenotype cells: {merged_cells_raw['cell_0'].duplicated().sum():,}"
    )
    ...
    if output_path is not None:
        try:
            ...
            raw_matches_path = output_path_str.replace(
                ".parquet", "_raw_matches.parquet"
            )
    ...
    print(
        f"Distance statistics: mean={merged_cells['distance'].mean():.2f}, max={merged_cells['distance'].max():.2f}"
    )
...
def debug_coordinate_transformation(pheno_coords, sbs_coords, rotation, translation):
    ...
    print(
        f"  i: [{pheno_coords[:, 0].min():.0f}, {pheno_coords[:, 0].max():.0f}] (range: {pheno_coords[:, 0].max() - pheno_coords[:, 0].min():.0f})"
    )
    print(
        f"  j: [{pheno_coords[:, 1].min():.0f}, {pheno_coords[:, 1].max():.0f}] (range: {pheno_coords[:, 1].max() - pheno_coords[:, 1].min():.0f})"
    )

    print(f"Transformed phenotype range:")
    print(
        f"  i: [{transformed_coords[:, 0].min():.0f}, {transformed_coords[:, 0].max():.0f}] (range: {transformed_coords[:, 0].max() - transformed_coords[:, 0].min():.0f})"
    )
    print(
        f"  j: [{transformed_coords[:, 1].min():.0f}, {transformed_coords[:, 1].max():.0f}] (range: {transformed_coords[:, 1].max() - transformed_coords[:, 1].min():.0f})"
    )

    print(f"SBS coordinate range:")
    print(
        f"  i: [{sbs_coords[:, 0].min():.0f}, {sbs_coords[:, 0].max():.0f}] (range: {sbs_coords[:, 0].max() - sbs_coords[:, 0].min():.0f})"
    )
    print(
        f"  j: [{sbs_coords[:, 1].min():.0f}, {sbs_coords[:, 1].max():.0f}] (range: {sbs_coords[:, 1].max() - sbs_coords[:, 1].min():.0f})"
    )
...
def geographic_constrained_sampling(
    ...
    print(
        f"Dataset boundaries: i=[{i_min:.0f}, {i_max:.0f}], j=[{j_min:.0f}, {j_max:.0f}]"
    )
    ...
    print(
        f"Distance range: {df['dist_from_center_norm'].min():.3f} to {df['dist_from_center_norm'].max():.3f}"
    )
    print(f"Cells at various distances:")
    print(f"  0-20%: {(df['dist_from_center_norm'] <= 0.2).sum():,}")
    print(
        f"  20-40%: {((df['dist_from_center_norm'] > 0.2) & (df['dist_from_center_norm'] <= 0.4)).sum():,}"
    )
    print(
        f"  40-60%: {((df['dist_from_center_norm'] > 0.4) & (df['dist_from_center_norm'] <= 0.6)).sum():,}"
    )
    print(
        f"  60-80%: {((df['dist_from_center_norm'] > 0.6) & (df['dist_from_center_norm'] <= 0.8)).sum():,}"
    )
    print(
        f"  80-100%: {((df['dist_from_center_norm'] > 0.8) & (df['dist_from_center_norm'] <= 1.0)).sum():,}"
    )
    ...
    print(
        f"Cells in center region (0%-{center_radius:.0%}): {len(center_cells):,} ({100 * len(center_cells) / len(cell_positions):.1f}% of total)"
    )
    ...
    print(
        f"Sampling efficiency: {100 * len(final_sample) / len(cell_positions):.1f}% of original"
    )
    ...
    if len(final_sample) > 0:
        ...
        print(
            f"Sample coverage: i={i_range_sample:.0f} ({100 * i_range_sample / i_range:.1f}%), j={j_range_sample:.0f} ({100 * j_range_sample / j_range:.1f}%)"
        )
...
def hardcoded_scale_alignment(
    ...
    print(
        f"Using {len(pheno_sample):,} phenotype and {len(sbs_sample):,} SBS cells for translation"
    )
    ...
    print(
        f"  Matches within 10px: {good_matches:,}/{len(min_distances):,} ({score:.1%})"
    )
...
def hardcoded_scale_robust_alignment(
    ...
    try:
        ...
        print(
            f"RANSAC coefficients (should be ~1.0): [{coef[0, 0]:.3f}, {coef[1, 1]:.3f}]"
        )
        ...
        print(
            f"Validation: {good_matches_count:,}/{len(min_distances_all):,} matches within 10px ({score:.1%})"
        )
...
def pure_scaling_alignment(
    ...
    transformed_pheno = (
        pheno_sample * scale_factor
    )  # Just multiply by scale, no translation

    print(f"Coordinate ranges after pure scaling:")
    print(
        f"  Original phenotype: i=[{pheno_sample[:, 0].min():.0f}, {pheno_sample[:, 0].max():.0f}], j=[{pheno_sample[:, 1].min():.0f}, {pheno_sample[:, 1].max():.0f}]"
    )
    print(
        f"  Scaled phenotype: i=[{transformed_pheno[:, 0].min():.0f}, {transformed_pheno[:, 0].max():.0f}], j=[{transformed_pheno[:, 1].min():.0f}, {transformed_pheno[:, 1].max():.0f}]"
    )
    print(
        f"  SBS sample: i=[{sbs_sample[:, 0].min():.0f}, {sbs_sample[:, 0].max():.0f}], j=[{sbs_sample[:, 1].min():.0f}, {sbs_sample[:, 1].max():.0f}]"
    )
    ...
    for thresh in thresholds:
        ...
        scores[thresh] = score
        print(
            f"  Matches within {thresh:2d}px: {good_matches:4d}/{len(min_distances):,} ({score:.1%})"
        )
