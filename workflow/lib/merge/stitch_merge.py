...
def load_alignment_parameters(alignment_row: pd.Series) -> Dict[str, Any]:
    ...
    if not isinstance(rotation_flat, (list, tuple, np.ndarray)) or len(rotation_flat) != 4:
...
def find_cell_matches(
    ...
    if memory_required_gb < 50:
        ...
        return _find_matches_direct(
            ...
            transformed_coords, sbs_coords,
            threshold, alignment.get("scale_factor", 1.0),
            verbose
        )
    else:
        if verbose:
...
def _find_matches_direct(
    ...
    matches_df = _build_matches_dataframe(
        phenotype_positions, sbs_positions,
        valid_pheno_idx, valid_sbs_idx, valid_distances,
        transformed_coords, sbs_coords
    )
    ...
    for chunk_idx in range(n_chunks):
        ...
        if valid_mask.sum() > 0:
            ...
            chunk_matches = _build_matches_dataframe(
                phenotype_positions, sbs_positions,
                chunk_pheno_idx, chunk_sbs_idx, chunk_distances,
                transformed_coords, sbs_coords
            )
...
def validate_matches(matches_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    good_quality = not has_duplicates and distances.mean() < 5.0 and over_20px < n_matches * 0.05
...
def filter_tiles_by_diversity(df: pd.DataFrame, data_type: str) -> pd.DataFrame:
    ...
    print(f"{data_type} filtering: {len(df):,} → {len(filtered_df):,} cells ({removed_tiles} tiles removed)")
...
def build_final_matches(
    ...
    pheno_id_map = phenotype_filtered.reset_index().set_index("stitched_cell_id")["index"].to_dict()
    sbs_id_map = sbs_filtered.reset_index().set_index("stitched_cell_id")["index"].to_dict()
    ...
    final_matches = pd.DataFrame({
        ...
        "area_0": phenotype_rows.get("area", pd.Series([np.nan] * len(pheno_indices))).values,
        "area_1": sbs_rows.get("area", pd.Series([np.nan] * len(sbs_indices))).values,
...
def create_merge_summary(
    ...
    if final_matches.empty:
        return _create_failure_summary(plate, well, threshold, phenotype_scaled, sbs_positions)
    ...
    summary.update({
        ...
        **{f"validation_distance_{k}": v for k, v in validation.get("distance_stats", {}).items()},
        **{f"validation_distribution_{k}": v for k, v in validation.get("distance_distribution", {}).items()},
        **{f"validation_quality_{k}": v for k, v in validation.get("quality_flags", {}).items()},
        **{f"validation_duplication_{k}": v for k, v in validation.get("duplication", {}).items()},
    })
...
def _log_validation_results(validation: Dict[str, Any]) -> None:
    ...
    # Quality assessment
    quality = "GOOD" if validation.get("quality_flags", {}).get("good_quality") else "ACCEPTABLE"
    print(f"✅ Match quality: {quality}")
    
    # Distance stats
    stats = validation.get("distance_stats", {})
    dist = validation.get("distance_distribution", {})
    print(f"Distance: mean={stats.get('mean', 0):.1f}px, max={stats.get('max', 0):.1f}px")
    print(f"Quality: {dist.get('under_5px', 0)} under 5px, {dist.get('under_10px', 0)} under 10px")
