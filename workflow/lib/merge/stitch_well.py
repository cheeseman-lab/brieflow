...
def metadata_to_grid_positions(
    metadata_df: pd.DataFrame, well: str
) -> Dict[int, Tuple[int, int]]:
    """
    Convert metadata to grid positions for tiles in a well with continuous stage coordinates.
    """
    return metadata_to_grid_positions_continuous(
        metadata_df, well, grid_spacing_tolerance=50.0
    )


def metadata_to_grid_positions_continuous(
    metadata_df: pd.DataFrame, well: str, grid_spacing_tolerance: float = 50.0
) -> Dict[int, Tuple[int, int]]:
    """
    Convert metadata to grid positions for tiles in a well with continuous stage coordinates.
    ...
    print(
        f"Grid dimensions: {max(pos[0] for pos in tile_positions.values()) + 1} x {max(pos[1] for pos in tile_positions.values()) + 1}"
    )

    return tile_positions


def pairwise_shifts_tiff(
    metadata_df: pd.DataFrame,
    well: str,
    flipud: bool = False,
    fliplr: bool = False,
...
def optimal_positions_tiff(
    ...
    x_guess = np.array(
        [tile_positions[tile_id][0] * tile_size[1] for tile_id in tile_ids],
        dtype=np.float32,
    )
    y_guess = np.array(
        [tile_positions[tile_id][1] * tile_size[0] for tile_id in tile_ids],
        dtype=np.float32,
    )
...
def assemble_tiff_well(
    ...
    for _, tile_row in tqdm(
        well_metadata.iterrows(), total=len(well_metadata), desc="Assembling tiles"
    ):
...
def pairwise_shifts_aligned_tiff(
    metadata_df: pd.DataFrame,
    well: str,
    data_type: str = "phenotype",
    flipud: bool = False,
    ...
    rot90: int = 0,
    channel: int = 0,
) -> Tuple[List[AlignedTiffEdge], Dict]:
    """
