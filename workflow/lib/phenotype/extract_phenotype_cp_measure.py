...
from itertools import permutations #used to generate pairs of channels for colocalization
...
def get_colocalization_features(image1, image2, mask, prefix=''):
    ...
    try:
        ...
        print(f"Warning: Error calling colocalization measurement {name} with {image1}, {image2}, and {mask}: {str(e)}")
...
def get_neighbor_features(mask1, mask2, prefix=''):
    ...
    try:
        ...
        print(f"Error calling neighbor measurment {name} with {mask1} and {mask2}: {str(e)}")
    return results
...
def extract_phenotype_cp_measure(
    ...
    try:
        ...
        for (ch1_idx, ch1_name), (ch2_idx, ch2_name) in permutations(enumerate(channel_names), 2): #FIX: using permutations instead of combinations
