...
def get_metadata_extraction_wildcards(
    image_type: str, config: dict, samples_df, metadata_samples_df
) -> pd.DataFrame:
    """Determine the appropriate wildcard combinations for metadata extraction.

    Uses config parameters to determine organization, then validates against available dataframes.

    Args:
        image_type: 'sbs' or 'phenotype'
        config: Configuration dictionary
        samples_df: DataFrame with image file paths and metadata
        metadata_samples_df: DataFrame with external metadata file paths

    Returns:
        DataFrame with appropriate wildcard combinations for metadata extraction
    """
    data_config = get_data_config(image_type, config)

    if not metadata_samples_df.empty:
        # Use external metadata file organization
        # The metadata dataframe structure determines the wildcards
        metadata_columns = [
        ...
        # Extract metadata from image files - use config to determine organization
        metadata_org = data_config["metadata_data_organization"]

        if metadata_org == "tile":
            # Tile-level metadata extraction
            if image_type == "sbs":
                required_cols = ["plate", "well", "tile", "cycle"]
            else:  # phenotype
                required_cols = ["plate", "well", "tile"]
                if "round" in samples_df.columns:
                    required_cols.append("round")
        else:
            # Well-level metadata extraction
            if image_type == "sbs":
                required_cols = ["plate", "well", "cycle"]
            else:  # phenotype
                required_cols = ["plate", "well"]
                if "round" in samples_df.columns:
                    required_cols.append("round")

        # Validate that required columns exist in samples dataframe
        available_cols = [col for col in required_cols if col in samples_df.columns]
        if len(available_cols) != len(required_cols):
            missing_cols = set(required_cols) - set(available_cols)
            raise ValueError(
                f"Missing required columns for {image_type} metadata extraction: {missing_cols}"
            )

        return samples_df[available_cols].drop_duplicates().astype(str)


def get_metadata_output_wildcards(
    image_type: str, config: dict, samples_df, metadata_samples_df
) -> dict:
    """Get the wildcard pattern for metadata extraction output files.

    Uses config parameters to determine the expected wildcard structure.
    """
    data_config = get_data_config(image_type, config)

    if not metadata_samples_df.empty:
        # Use metadata file structure for output pattern
        metadata_columns = [
            col for col in metadata_samples_df.columns if col != "sample_fp"
        ]
        return {col: f"{{{col}}}" for col in metadata_columns}

    else:
        # Use config-determined organization for image-based metadata
        metadata_org = data_config["metadata_data_organization"]
        ...
        if metadata_org == "tile":
            if image_type == "sbs":
                ...
                pattern = {"plate": "{plate}", "well": "{well}", "tile": "{tile}"}
                if "round" in samples_df.columns:
                    pattern["round"] = "{round}"
        else:
            # Well-level organization
            if image_type == "sbs":
                pattern = {"plate": "{plate}", "well": "{well}", "cycle": "{cycle}"}
            else:  # phenotype
                pattern = {"plate": "{plate}", "well": "{well}"}
                if "round" in samples_df.columns:
                    pattern["round"] = "{round}"
...
def should_use_samples_input(
    image_type: str, config: dict, metadata_samples_df
) -> bool:
    """Determine if metadata extraction rules need sample files as input.
    ...
    Returns:
        True if sample files are needed for metadata extraction
    """
    return metadata_samples_df.empty  # Only need samples if no external metadata exists


def should_use_metadata_input(
    image_type: str, config: dict, metadata_samples_df
) -> bool:
    """Determine if metadata extraction rules need metadata files as input.
    ...
    Returns:
        True if metadata files are needed for metadata extraction
    """
    return not metadata_samples_df.empty


def get_sample_input_wildcards(image_type: str, config: dict) -> list:
    """Get the wildcards needed for sample file input selection.

    This uses the image data organization since we're selecting image files.
    """
    data_config = get_data_config(image_type, config)
    image_org = data_config["image_data_organization"]

    if image_org == "tile":
        base_wildcards = ["plate", "well", "tile"]
    else:
        base_wildcards = ["plate", "well"]

    if image_type == "sbs":
        base_wildcards.append("cycle")

    return base_wildcards


def get_metadata_input_wildcards(
    image_type: str, config: dict, metadata_samples_df
) -> list:
    """Get the wildcards needed for metadata file input selection.

    This is determined by the structure of the metadata samples dataframe.
    """
    if metadata_samples_df.empty:
        return []

    return [col for col in metadata_samples_df.columns if col != "sample_fp"]
