...
def standardize_barcode_design(
    ...
    nontargeting_patterns: List[str] = ["nontargeting", "NTC", "sg_nt", "non-targeting", "negative"],
    ...
    if uniprot_data_path is None:
        raise ValueError("uniprot_data_path is required for gene validation. "
    ...
    if barcode_col not in df.columns:
        raise ValueError(f"Barcode column '{barcode_col}' not found in design table. "
    ...
    if gene_symbol_col is not None and gene_symbol_col in df.columns:
        ...
        if verbose:
            print(f"Created gene_symbol column with value: '{df['gene_symbol'].iloc[0] if len(df) > 0 else ''}'")
    ...
    if gene_id_col is not None and gene_id_col in df.columns:
        ...
        warnings.warn(f"Gene ID column '{gene_id_col}' not found. Skipping gene_id column.")
    ...
    if df["prefix"].isna().any():
        warnings.warn("Some prefixes could not be generated. Check your prefix function or barcode data.")
    
    # Handle ampersand genes if requested
    if handle_ampersand_genes and "gene_symbol" in df.columns:
        df = simplify_ampersand_genes(df, gene_symbol_col="gene_symbol", verbose=verbose)
    ...
    # Add UniProt annotation (REQUIRED) - but skip non-targeting controls
    ...
    if len(regular_genes) > 0:
        ...
        if unannotated_count > 0:
            unannotated_genes = regular_genes[regular_genes["uniprot_entry"].isna()]["gene_symbol"].unique()
            warnings.warn(f"{unannotated_count} genes could not be validated against UniProt: "
    ...
    if verbose:
        ...
        nontargeting_count = df["gene_symbol"].str.startswith("nontargeting_", na=False).sum()
...
def simplify_ampersand_genes(df: pd.DataFrame, gene_symbol_col: str = "gene_symbol", verbose: bool = True) -> pd.DataFrame:
    """
    ...
    if gene_symbol_col not in result_df.columns:
        warnings.warn(f"Column '{gene_symbol_col}' not found. No ampersand processing performed.")
        return result_df
    ...
    if mask.any():
        ...
        if verbose:
            ...
            print(f"  - This represents dual-targeting guides analyzed by primary target")
    
    return result_df


def load_and_process_uniprot_data(uniprot_data_path: str, verbose: bool = True) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if verbose and len(uniprot_data) < initial_count:
            print(f"Removed {initial_count - len(uniprot_data)} entries without gene names")
        ...
        uniprot_processed = expanded_df.sort_values(["gene_name", "position"]).drop_duplicates(
        ...
        raise ValueError(f"Error processing UniProt data from {uniprot_data_path}: {str(e)}")
...
def standardize_nontargeting_controls(
    ...
    if gene_symbol_col not in result_df.columns:
        warnings.warn(f"Column '{gene_symbol_col}' not found. No non-targeting standardization performed.")
        return result_df
    
    if prefix_col not in result_df.columns:
        warnings.warn(f"Column '{prefix_col}' not found. Cannot generate standardized non-targeting names.")
        return result_df
    ...
    nontargeting_mask = result_df[gene_symbol_col].astype(str).str.contains(
        pattern_regex, na=False, regex=True, case=False
    )
...
def add_uniprot_annotation(df: pd.DataFrame, uniprot_data_path: str, verbose: bool = True) -> pd.DataFrame:
    """
    ...
    # Identify non-targeting controls
    nontargeting_mask = df["gene_symbol"].astype(str).str.startswith("nontargeting_", na=False)
    
    # Merge with gene symbols for non-control genes only
    df_genes = df[~nontargeting_mask].copy() if nontargeting_mask.any() else df.copy()
    df_controls = df[nontargeting_mask].copy() if nontargeting_mask.any() else pd.DataFrame()
    
    # Annotate regular genes
    if len(df_genes) > 0:
        df_genes_annotated = df_genes.merge(
        ...
        df_genes_annotated = pd.DataFrame()
    
    # Annotate non-targeting controls with special identifier
    if len(df_controls) > 0:
        df_controls["uniprot_entry"] = "NON_TARGETING_CONTROL"
    
    # Combine back together
    if len(df_controls) > 0 and len(df_genes_annotated) > 0:
        df_annotated = pd.concat([df_genes_annotated, df_controls], ignore_index=True)
        # Restore original order
        df_annotated = df_annotated.reindex(df.index.tolist())
    elif len(df_controls) > 0:
        df_annotated = df_controls
    else:
        df_annotated = df_genes_annotated
    
    # Check annotation success for regular genes
    if len(df_genes) > 0:
        regular_genes = df_annotated[~df_annotated["gene_symbol"].str.startswith("nontargeting_", na=False)]
        annotated_count = regular_genes["uniprot_entry"].notna().sum()
        unannotated_genes = regular_genes[regular_genes["uniprot_entry"].isna()]["gene_symbol"].unique()
        
        if verbose:
            print(f"UniProt annotation results:")
            print(f"  - {annotated_count}/{len(regular_genes)} regular genes successfully annotated")
            
            if len(df_controls) > 0:
                print(f"  - {len(df_controls)} non-targeting controls marked as NON_TARGETING_CONTROL")
            
            if len(unannotated_genes) > 0:
                print(f"  - {len(unannotated_genes)} unique genes without UniProt annotation:")
                print(f"    {list(unannotated_genes)[:10]}{'...' if len(unannotated_genes) > 10 else ''}")
                print(f"  - Consider reviewing these genes or updating gene symbols")
    elif len(df_controls) > 0 and verbose:
        # Only non-targeting controls case
        print(f"UniProt annotation results:")
        print(f"  - {len(df_controls)} non-targeting controls marked as NON_TARGETING_CONTROL")
        print(f"  - No regular genes to validate")
    
    return df_annotated


def validate_gene_symbols(df: pd.DataFrame, uniprot_data_path: Optional[str] = None, 
                         gene_symbol_col: str = "gene_symbol") -> pd.DataFrame:
    """
    ...
    if uniprot_data_path is not None:
        ...
        for gene in unique_genes:
            ...
            if is_valid:
                ...
                recommendation = "Consider checking gene symbol or removing from analysis"
        ...
        for gene in unique_genes:
            ...
            if has_ampersand:
                ...
                recommendation = "Consider converting to uppercase (standard gene naming)"
            elif has_numbers:
...
def create_concatenation_prefix(col1: str, col2: str, col1_slice: Optional[slice] = None):
