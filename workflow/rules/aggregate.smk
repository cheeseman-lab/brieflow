...
rule split_datasets:
    ...
    params:
        ...
        metadata_cols_fp=config.get("classifier", {}).get("METADATA_COLS_FP") or config["aggregate"]["metadata_cols_fp"],
        classifier_path=config.get("classifier", {}).get("CLASSIFIER_PATH"),
        confidence_thresholds=config.get("classifier", {}).get("CONFIDENCE_THRESHOLDS"),
...
rule filter:
    ...
    params:
        ...
        use_classifier=config.get("classifier", {}).get("CLASSIFIER_PATH") is not None,
