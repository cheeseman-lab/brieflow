...
# Load construct data to get construct ID (sgRNA ID)
construct_data = pd.read_csv(snakemake.input.construct_data, sep="\t")
construct_id = construct_data["construct_id"].iloc[0]  # This is the sgRNA ID
...
# Get feature names (excluding first column which is sgRNA ID)
...
# Get sample size for this construct (number of cells for this sgRNA)
construct_mask = (
    sample_sizes_df.iloc[:, 0] == construct_id
)  # First column is construct ID
...
# Reorder columns to put gene and construct info first
...
# Save null distribution as numpy (for gene-level aggregation)
