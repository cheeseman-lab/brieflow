...
filtered_dataset = ds.dataset(snakemake.input.filtered_paths[:1], format="parquet")
perturbation_col = filtered_dataset.to_table(columns=[pert_col]).to_pandas()[
    pert_col
]
...
#         nontargeting_indices, size=min(len(nontargeting_indices), len(gene_indices)*NONTARGETING_CONTROL_MULTIPLIER), replace=False
#     )
...
#     perturbation_scores = get_perturbation_score(
...
filtered_dataset = filtered_dataset.to_table(use_threads=True, memory_pool=None).to_pandas()
filtered_dataset.insert(len(metadata_cols), "perturbation_score", perturbation_scores_col)
