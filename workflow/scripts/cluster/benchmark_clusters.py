...
def run_benchmark_analysis(
    ...
    for dataset_name in cluster_datasets.keys():
        ...
        pie_charts[dataset_name] = plot_combined_enrichment_pie(
            integrated_results, dataset_name
        )

        # Generate cluster enrichment plots if the table exists
        # Skip for shuffled data
        if dataset_name in combined_tables and "shuffled" not in dataset_name.lower():
            cluster_enrichment_plots[dataset_name] = plot_cluster_enrichments(
...
def plot_combined_enrichment_pie(integrated_results, dataset_name):
...
def plot_cluster_enrichments(cluster_enrichment_df):
    """Creates a combined plot:
    - Stacked CORUM and KEGG enrichments (primary axis)
    - STRING true positives (secondary axis, as bars)

    Args:
        cluster_enrichment_df (pandas.DataFrame): DataFrame with 'cluster', 'STRING_true_positives',
            'CORUM_enriched_groups', and 'KEGG_enriched_groups' columns (all numeric).

    Returns:
        matplotlib.figure.Figure: Figure object that can be saved or displayed.
    """
    plot_df = cluster_enrichment_df.copy()

    # Fill NaNs with 0
    plot_df["STRING_true_positives"] = plot_df["STRING_true_positives"].fillna(0)
    plot_df["CORUM_enriched_groups"] = plot_df["CORUM_enriched_groups"].fillna(0)
    plot_df["KEGG_enriched_groups"] = plot_df["KEGG_enriched_groups"].fillna(0)

    # Sort by cluster
    plot_df = plot_df.sort_values("cluster")

    # Set up plot with high DPI
    fig, ax1 = plt.subplots(figsize=(15, 4), dpi=300)  # Added high DPI setting

    bar_width = 0.25

    # Define x positions for each set of bars
    x = np.arange(len(plot_df))

    # Primary axis: CORUM and KEGG enrichments (stacked bars)
    ax1.bar(
        x - bar_width / 2,
        plot_df["CORUM_enriched_groups"],
        label="CORUM enriched groups",
        color="#ff7f0e",
        width=bar_width,
    )
    ax1.bar(
        x - bar_width / 2,
        plot_df["KEGG_enriched_groups"],
        bottom=plot_df["CORUM_enriched_groups"],
        label="KEGG enriched groups",
        color="#2ca02c",
        width=bar_width,
    )
    ax1.set_ylabel("Number of Enriched CORUM+KEGG Groups", color="black", fontsize=12)

    # Secondary axis: STRING true positives
    ax2 = ax1.twinx()
    ax2.bar(
        x + bar_width / 2,
        plot_df["STRING_true_positives"],
        label="STRING true positives",
        color="#1f77b4",
        width=bar_width,
    )
    ax2.set_ylabel("STRING True Positives", color="black", fontsize=12)
    ax2.tick_params(axis="y", labelcolor="#1f77b4")

    # X-axis setup
    ax1.set_xlabel("Cluster", fontsize=12)
    ax1.set_xticks(x)
    ax1.set_xticklabels(plot_df["cluster"], rotation=90, fontsize=8)

    # Remove unnecessary white space before first and after last cluster
    ax1.set_xlim(-0.5, len(plot_df) - 0.5)

    # Title and legend
    plt.title("Cluster Enrichments", fontsize=16)
