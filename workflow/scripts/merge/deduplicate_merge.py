...
if distance_analysis:
    ...
    print(
        f"<5px precision: {distance_analysis['distance_distribution']['under_5px']}/{len(merge_deduplicated)} ({distance_analysis['distance_distribution']['under_5px'] / len(merge_deduplicated) * 100:.1f}%)"
    )
