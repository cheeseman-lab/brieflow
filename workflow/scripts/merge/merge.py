...
missing_columns = [
    col for col in expected_columns if col not in well_merge_data.columns
]
if missing_columns:
...
if len(well_merge_data) > 0:
    ...
    missing_critical = [
        col for col in critical_cols if col not in well_merge_data.columns
    ]

    if missing_critical:
        print(
            f"ERROR: Missing critical columns for downstream processing: {missing_critical}"
        )
    else:
    ...
    print(
        well_merge_data[
            ["plate", "well", "cell_0", "cell_1", "tile", "site", "distance"]
        ].head()
    )
