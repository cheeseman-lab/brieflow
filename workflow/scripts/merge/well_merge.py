...
def main():
    ...
    phenotype_triangles = well_level_triangle_hash(
        phenotype_scaled
    ...
    if len(phenotype_triangles) == 0 or len(sbs_triangles) == 0:
        ...
        pd.DataFrame(columns=["V_0", "V_1", "c_0", "c_1", "magnitude"]).to_parquet(
            str(snakemake.output.phenotype_triangles)
        )
        pd.DataFrame(columns=["V_0", "V_1", "c_0", "c_1", "magnitude"]).to_parquet(
            str(snakemake.output.sbs_triangles)
        )
    ...
    print(
        f"✅ Saved scaled phenotype triangles: {snakemake.output.phenotype_triangles}"
    )
    ...
    phenotype_transformed.to_parquet(str(snakemake.output.phenotype_transformed))
    print(
        f"✅ Saved transformed phenotype positions: {snakemake.output.phenotype_transformed}"
    )
        ...
        [
            {
                ...
                "overlap_fraction": safe_float(
                    best_alignment.get("overlap_fraction", overlap_fraction)
                ),
    ...
    print(
        f"   Matches < 5px: {(merged_cells['distance'] < 5).sum():,} ({(merged_cells['distance'] < 5).mean() * 100:.1f}%)"
    )
    print(
        f"   Matches < 10px: {(merged_cells['distance'] < 10).sum():,} ({(merged_cells['distance'] < 10).mean() * 100:.1f}%)"
    )
    ...
    summary_stats = {
        ...
        "step3_alignment": {
            ...
            "scale_factor_used": safe_float(
                best_alignment.get("scale_factor", scale_factor)
            ),
        ...
        "step4_cell_merging": {
            ...
            "match_rate_phenotype": float(
                len(merged_cells_final) / len(phenotype_positions)
            ),
