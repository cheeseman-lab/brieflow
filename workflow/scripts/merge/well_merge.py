...
print(
    f"After filtering: {len(phenotype_well)} phenotype cells, {len(sbs_well)} SBS cells"
)
...
if (
    ...
    print(
        f"   Determinant: {alignment['determinant']:.3f} (required: {snakemake.params.det_range[0]}-{snakemake.params.det_range[1]})"
    )
...
try:
    ...
    missing_required = [
        col for col in required_for_format_merge if col not in merged_cells.columns
    ]
