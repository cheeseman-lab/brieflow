...
def main():
    ...
    try:
        ...
        print(f"Processing Well {plate}-{well}: {len(raw_matches):,} raw matches → {len(merged_cells):,} simple matches")
        ...
        # Report results
        stitched_status = "✅ 1:1 stitched mapping" if validation_results['is_1to1_stitched'] else "⚠️  Stitched duplicates present"
        print(f"Deduplication complete: {len(final_output):,} matches ({quality_metrics['quality_tier']} quality)")
        print(f"Spatial validation: {stitched_status}, Mean distance: {quality_metrics['mean_distance']:.2f}px, "
              f"<5px precision: {quality_metrics['precision_5px']:.1%}")
        
        # Warn about poor quality matches
        if quality_metrics['large_distance_count'] > 0:
            print(f"⚠️  {quality_metrics['large_distance_count']} matches >50px may indicate alignment issues")
